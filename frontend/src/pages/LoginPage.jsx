...
const LoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      toast.success('Logged in successfully as worker');
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-6 text-primary">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-secondary font-medium uppercase tracking-[0.2em] text-[10px]">The Modern Curator of Protection</p>
        </div>

        <div className="bg-white rounded-[3rem] p-10 border border-border/10 shadow-sm">
          <form onSubmit={handleLogin} className="space-y-8">
            <h2 className="text-4xl font-serif italic text-primary">Welcome back</h2>
            
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-secondary">Email / Phone</label>
                <input 
                  type="text"
            ...
            >
              ...
              <span>Login to Worker Portal</span>
            </button>

            <p className="text-center text-[10px] text-secondary font-bold uppercase tracking-widest">
              Don't have an account? <Link to="/onboarding" className="text-primary hover:underline">Register here</Link>
            </p>
          </form>
        </div>
      </div>
